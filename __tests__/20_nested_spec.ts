...
describe('two-level nested spec', () => {
  type State = {
    a: number;
    b: number;
    c: number;
  ...
  const computeScore = memoize((state: State) => ({
    result: state.a + state.b,
  }));

  const getScore = memoize((state: State) => ({
    value: computeScore(state),
  }));
...
describe('two-level nested spec with different intermediates', () => {
  type State = {
    a: number;
    b: number;
    c: number;
